...
In this first part of the tutorial, we will start by learning how to create a URDF. URDF stands for Universal Robot Description File. The file is written in XML and is a way to describe the links, joints, shape, and movement limits.  Most robot description files you will encounter online are written not a URDFs but as XACROs or XML macros. Macros are an easy way to separate a URDF into parts so that the URDF of a complex robot isn't extremely long and difficult to parse.  For example, there may be a macro for links and joints, and another for the transmission conrolling rate of joint rotation for all joints. The macros are referenced by a 'main' XACRO that arranges them in their proper order.
